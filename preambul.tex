...
%ОГЛАВЛЕНИЕ
\makeatletter
\def\tableofcontentsname{Оглавление} % Наименование раздела Оглавление(Содержание)
%\def\contentsname{Оглавление} % Стандартная переменная, с пакетом babel почему-то не работает переопределение
\renewcommand{\tableofcontents}{\section*{\centering\tableofcontentsname}\@starttoc{toc}}
...
%РАЗДЕЛЫ
%\usepackage{titlesec}
%\titleformat{\section}{\centering\uppercase}{\thesection}{}{}
%\titleformat{name=\section,page=even}[leftmargin]{\filleft\scshape}{\thesection}{.5em}{}
\makeatletter
\renewcommand{\@seccntformat}[1]{\csname the#1\endcsname~} % Убрать в названии секций точку после номера
